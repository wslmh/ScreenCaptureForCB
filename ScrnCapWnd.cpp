...
BOOL CScrnCapWnd::AutoRecognizeWnd(const POINT& ptMousePos)
{
	...
	std::vector<RectX>::iterator itWndRc = m_vecAllWndRect.begin();
...
BOOL CScrnCapWnd::Save()
{
	...
	strcpy(szDefPath, strDefName.c_str());
	...
	if(GetSaveFilePath(m_hInstance,GetSafeHwnd(),szDefPath,szDefTitle,nFilterIndex))
	{
		...
		AnsiString strFileName = szDefPath;
...
int CScrnCapWnd::DrawTextEx(HDC hDC, const AnsiString& strTx, RectX rc, UINT uFormat)
{
...
{
	AnsiString strRcSize;
	strRcSize.Format(_T("%d×%d"), ARRAYOFCONST(( abs(m_rcSel.GetW()), abs(m_rcSel.GetH()) )));
	...
	SIZE sizeString;
	GetTextExtentPoint32(hDC, strRcSize.c_str(), strRcSize.Length(), &sizeString);
...
BOOL CALLBACK CScrnCapWnd::EnumGrandChildWindowsProc(HWND hwnd, LPARAM lParam)
{
	InsertWndRect(hwnd);
	return TRUE;
}
...
BOOL CALLBACK CScrnCapWnd::EnumChildWindowsProc(HWND hwnd, LPARAM lParam)
{
...
BOOL CALLBACK CScrnCapWnd::EnumWindowsProc(HWND hwnd,LPARAM lParam)
{
...
void CScrnCapWnd::GetCurTimeString(AnsiString& strCurDateTime)
{
	SYSTEMTIME systime;
	GetLocalTime(&systime);
	strCurDateTime.Format(_T("%4d%02d%02d%02d%02d%02d"),
		                  ARRAYOFCONST(( systime.wYear,systime.wMonth, systime.wDay,
		                  systime.wHour, systime.wMinute, systime.wSecond )) );
}
//---------------------------------------------------------------------------

void CScrnCapWnd::SetDefFileName(AnsiString& strDefName)
{
	strDefName = _T("截图");
	AnsiString strTime;
	GetCurTimeString(strTime);
	//strDefName.Append(strTime);
    strcat(strDefName.c_str(), strTime.c_str());
}
//---------------------------------------------------------------------------
...
BOOL CScrnCapWnd::SaveAsPic(HBITMAP hImage, const AnsiString& strFileName, const AnsiString& strFormat)
{
	BOOL bRes = FALSE;
	...
	{
		CLSID picClsid;
        UnicodeString strWCFormat = strFormat;
        UnicodeString strWCName = strFileName;
		CScrnCapWnd::GetEncoderClsid(strWCFormat.c_str(), &picClsid);
		Bitmap *pBitmap = Bitmap::FromHBITMAP(hImage,NULL);
		bRes = Gdiplus::Ok == pBitmap->Save(strWCName.c_str(), &picClsid);
	}
